...
function OurService() {
  ...
  return (
    <div className='services px-36 pt-4 '>
        <h1 className='text-4xl font-bold '>Our Service</h1>
        <div className='flex flex-row gap-8 overflow-scroll scrollbar-hide'>
            {data.map((item,index)=>(
                <div className='cursor-pointer min-width pt-4 pb-4 dark:bg-gray-900 flex flex-col items-center rounded-xl'>
                <img className='w-full object-cover' src={item.img} alt="" />
                <p className='text-white font-bold text-xl mt-2'>{item.name}</p>
        </div>
            ))}
        </div>
    </div>
